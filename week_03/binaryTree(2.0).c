...
void printTree_rec(treenode *root, int level)
{
    ...
    printTree_rec(root->left, level + 1); // here we send the new notree pointer that is n1->left = n2; //11
    printTabs(level);
    printf("right\n");

    printTree_rec(root->right, level + 1); // here we send the new notree pointer that is n1->left = n2;  //11
